...
username = text:match('^رفع ادمن @(.*)')
function prom_reply(extra,result,success) database:sadd(SUDO..'mods:'..msg.chat_id_,result.sender_user_id_)
local user = result.sender_user_id_
if database:get('lang:gp:'..msg.chat_id_) then
SendMetion(msg.chat_id_, user, msg.id_, '• User [ '..user..' ] Was Added To The Group Promote List !', 9, string.len(user))
else
...
if database:get('lang:gp:'..msg.chat_id_) then
bot.sendMessage(msg.chat_id_, msg.id_, 1, '🎫¦ ID : [ `'..msg.sender_user_id_..'` ]\n🏆¦ Rank : [ '..t..' ]\n⚠️¦ Warn : [ `'..nwarn ..'` ]\n📨¦ msgs : [ `'..msgs..'` ] \n\n', 1, 'md')
else
...
----------------------------------------------------------------------------
if text and text:match("^meuser$") or text:match("^معرفي$") then
text = 'BOT INLINE'
...
if database:get('lang:gp:'..msg.chat_id_) then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,'🎫¦ ID : [ '..msg.sender_user_id_..' ]\n🏆¦ Rank : [ '..t..' ]\n📨¦ msgs : [ '..msgs..' ]\n↫📸¦ photo : [ '..result.total_count_..' ]\n️👁‍🗨¦ ch sudo : '..botchannel..'', 1, 'md')
else
