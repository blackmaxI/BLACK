...
database:del(hash)
bot.sendMessage(msg.chat_id_, msg.id_, 1, '✪ قفل ( `'..text..'` ) ↫ معطل  ',1,'md')
end
end
------------------------------------------------------------
...
if database:get('lang:gp:'..msg.chat_id_) then
bot.sendMessage(msg.chat_id_, msg.id_, 1, '↫ احصائيات البوت :\n\n⇜ الكروبات : [ `'..gps..'` ]\n⇜ عدد رسائل الكروبات : [ `'..sgpsm..'` ]\n\n⇜ الكروبات السوبر : [ `'..gp..'` ]\n⇜ رسائل كروبات السوبر : [ `'..gpsm..'` ]\n\n⇜ الاعضاء : [ `'..users..'` ]\n⇜ رسائل الاعضاء : [ `'..pvm..'` ]\n\n⇜ عدد جميع الرسائل : [ `'..allmgs..'` ]\n\n ', 1, 'md')
else
bot.sendMessage(msg.chat_id_, msg.id_, 1, '↫ احصائيات البوت :\n\n⇜ الكروبات السوبر: [ `'..gps..'` ]\n⇜ عدد رسائل الكروبات السوبر  : [ `'..sgpsm..'` ]\n\n⇜ الكروبات  : [ `'..gp..'` ]\n⇜ رسائل كروبات  : [ `'..gpsm..'` ]\n\n⇜ الاعضاء : [ `'..users..'` ]\n⇜ رسائل الاعضاء : [ `'..pvm..'` ]\n\n⇜ عدد جميع الرسائل : [ `'..allmgs..'` ]\n\n ', 1, 'md')
end
end
----------------------------------------------------------------------------
end -- end is_sudo msg
...
SendMetion(msg.chat_id_, result.sender_user_id_, msg.id_, '↫ العضو ( '..result.sender_user_id_..' )  به دلیل رعایت نکردن قوانین از مدیریت ربات اخطار دریافت کردید تعداد اخطار های شما :  '..(nwarn + 1)..'/'..wmax..'', 10, string.len(result.sender_user_id_))
end
end
end
end
...
----------------------------------------------------------------------------
if text and (text:match('^hellp') or text:match('^الاواامر$'))and check_user(msg) and not msg.forward_info_ then
text = [[
...
----------------------------------------------------------------------------
if text == 'mutechat' or text == 'فتح المحادثه' then
database:set(SUDO..'muteall'..msg.chat_id_,true)
if database:get('lang:gp:'..msg.chat_id_) then
...
bot.sendMessage(msg.chat_id_, msg.id_, 1, '↫  يمكن يتنضيف اكثر من ( 100 ) ', 1, 'md')
end
else
function cb(a,b,c)
...
if text and text:match('^lhelp$') or text:match("^م1$") then
